...
class Mmu:
    ...
    def __init__(self, config):
        ...
        # by me
        self.gcode.register_command('MMU_SYNC_SELECTOR_MOTOR', self.cmd_MMU_SYNC_SELECTOR_MOTOR, desc=self.cmd_MMU_SYNC_SELECTOR_MOTOR_help)
        self.gcode.register_command('MMU_SYNC_TOOL', self.cmd_MMU_SYNC_TOOL, desc=self.cmd_MMU_SYNC_TOOL_help)
        self.gcode.register_command('MMU_MOVE_SELECTOR', self.cmd_MMU_MOVE_SELECTOR, desc = self.cmd_MMU_MOVE_SELECTOR_help)
        self.gcode.register_command('MMU_HOME_SELECTOR', self.cmd_MMU_HOME_SELECTOR, desc = self.cmd_MMU_HOME_SELECTOR_help)
        self.gcode.register_command('MMU_LOAD_TOOL', self.cmd_MMU_LOAD_TOOL, desc = self.cmd_MMU_LOAD_TOOL_help)
        self.gcode.register_command('MMU_UNLOAD_TOOL', self.cmd_MMU_UNLOAD_TOOL, desc = self.cmd_MMU_UNLOAD_TOOL_help)
        self.gcode.register_command('MMU_ADJUST_CURRENT', self.cmd_MMU_ADJUST_CURRENT, desc = self.cmd_MMU_ADJUST_CURRENT_help)
    ...
    def _setup_mmu_hardware(self, config):
        ...
        if config.has_section(section):
            ...
            #config.fileconfig.set(section, 'position_min', -10000)
            #config.fileconfig.set(section, 'position_max', self._get_max_selector_movement())
            #config.fileconfig.set(section, 'position_max', 10000)
        ...
        for name in [self.ENDSTOP_TOOLHEAD, self.ENDSTOP_GATE, self.ENDSTOP_EXTRUDER]:
            ...
            if sensor is not None:
                ...
                # by me self.sensors[name].runout_helper.runout_gcode = None
                # by me self.sensors[name].runout_helper.insert_gcode = None
                ...
                # by me add extra endstop
                mcu_endstop_sel = self.selector_rail.add_extra_endstop(sensor_pin, name)
    ...
    def handle_connect(self):
        ...
        # by me
        self.selector_default_run_current = self.selector_tmc.get_status(0)['run_current'] if self.selector_tmc else None
        ...
        # by me
        self.selector_percentage_run_current = self.selector_restore_percent_run_current = 100
...
### SERVO AND MOTOR GCODE FUNCTIONS
    ...
    def cmd_MMU_MOTORS_OFF(self, gcmd):
        motor = gcmd.get('MOTOR', "all").lower()
        if self._check_is_disabled(): return
        self._motors_off(motor)
        #self._servo_move()
        #self._servo_reset_state()
    ...
    cmd_MMU_SYNC_SELECTOR_MOTOR_help = "Sync the MMU selector motor to the extruder stepper"
    def cmd_MMU_SYNC_SELECTOR_MOTOR(self, gcmd):
        if self._check_is_disabled(): return
        if self._check_in_bypass(): return
        #servo = gcmd.get_int('SERVO', 1, minval=0, maxval=1)
        sync = gcmd.get_int('SYNC', 1, minval=0, maxval=1)
        force_in_print = gcmd.get_int('FORCE_IN_PRINT', 0, minval=0, maxval=1)
        #self._sync_gear_to_extruder(sync, servo=servo, current=self._is_in_print(force_in_print))
        prev_sync_state = self.mmu_toolhead.is_selector_synced_to_extruder()
        if prev_sync_state != sync:
            self._log_debug("%s selector stepper and extruder" % ("Syncing" if sync else "Unsyncing"))
            self.mmu_toolhead.sync_selector_to_extruder(self.extruder_name if sync else None)
            self.printer.send_event("mmu:extruder_synced" if sync else "mmu:extruder_unsynced")
        else:
            raise gcmd.error("Sync state didn't change")

    cmd_MMU_SYNC_TOOL_help = "Sync the MMU tool"
    def cmd_MMU_SYNC_TOOL(self, gcmd):
        if self._check_is_disabled(): return
        if self._check_in_bypass(): return
        tool = gcmd.get_int('TOOL', 1)
        tool1_sync_state = self.mmu_toolhead.is_selector_synced_to_extruder()
        tool2_sync_state = self.mmu_toolhead.is_gear_synced_to_extruder()
        if tool == 1:
            if tool2_sync_state:
                self.mmu_toolhead.sync_gear_to_extruder(None)
                #self._sync_gear_to_extruder(0, servo=0, current=self._is_in_print(0))
            if not tool1_sync_state:
                self.mmu_toolhead.sync_selector_to_extruder(self.extruder_name)
            #self._adjust_selector_current(self.sync_gear_current, "for extruder syncing")
            #self._restore_gear_current()
        elif tool == 2:
            if tool1_sync_state:
                self.mmu_toolhead.sync_selector_to_extruder(None)
            if not tool2_sync_state:
                self.mmu_toolhead.sync_gear_to_extruder(self.extruder_name)
                #self._sync_gear_to_extruder(1, servo=0, current=self._is_in_print(0))
            #self._adjust_gear_current(self.sync_gear_current, "for extruder syncing")
            #self._restore_selector_current()
        else:
            if tool1_sync_state:
                self.mmu_toolhead.sync_selector_to_extruder(None)
            if tool2_sync_state:
    ...
    cmd_MMU_ADJUST_CURRENT_help = "Reduce the current by tool num"
    def cmd_MMU_ADJUST_CURRENT(self, gcmd):
        if self._check_is_disabled(): return
        if self._check_in_bypass(): return
        tool = gcmd.get_int('TOOL', 1)
        if tool == 1:
...
#########################################################
    ...
    cmd_MMU_MOVE_SELECTOR_help = "Move selector"
    def cmd_MMU_MOVE_SELECTOR(self, gcmd):
        length = gcmd.get_float('LENGTH', self.calibrated_bowden_length)
        speed = gcmd.get_float('SPEED', 100)
        accel = gcmd.get_float('ACCEL', 500)
        try:
            self._set_selector_pos(0)
            self._trace_selector_move(None, length, speed, accel=accel)
        except MmuError as ee:
            raise gcmd.error("MMU_MOVE_SELECTOR: %s" % str(ee))

    cmd_MMU_HOME_SELECTOR_help = "Home selector"
    def cmd_MMU_HOME_SELECTOR(self, gcmd):
        length = gcmd.get_float('LENGTH', self.calibrated_bowden_length)
        extralength = gcmd.get_float('EXTRALENGTH', 6)
        speed = gcmd.get_float('SPEED', 100)
        accel = gcmd.get_float('ACCEL', 500)
        try:
            self._set_selector_pos(0)
            self._trace_selector_move(None, length, speed, homing_move=1, endstop_name=self.extruder_homing_endstop, accel=accel)
        except MmuError as ee:
            raise gcmd.error("MMU_MOVE_SELECTOR: %s" % str(ee))

    cmd_MMU_LOAD_TOOL_help = "Load tool"
    def cmd_MMU_LOAD_TOOL(self, gcmd):
        length = gcmd.get_float('LENGTH', self.calibrated_bowden_length)
        extralength = gcmd.get_float('EXTRALENGTH', 6)
        tool = gcmd.get_int('TOOL', 1)
        hominglength = gcmd.get_float('HOMINGLENGTH', 2000)
        homingspeed = gcmd.get_float('HOMINGSPEED', 200)
        speed = gcmd.get_float('SPEED', 200)
        accel = gcmd.get_float('ACCEL', 100)
        try:
            # restore current
            self._restore_gear_current()
            self._restore_selector_current()
            ...
            # Unsync
            if self.mmu_toolhead.is_selector_synced_to_extruder():
                self.mmu_toolhead.sync_selector_to_extruder(None)
            ...
            if tool == 1:
                if (length > 0):
                    self._set_selector_pos(0)
                    self._trace_selector_move(None, length, speed, accel=accel, wait=False)
                    #self._random_failure()
                    #self._movequeues_wait_moves()

                self._set_selector_pos(0)
                self._trace_selector_move(None, hominglength, homingspeed, homing_move=1, endstop_name=self.extruder_homing_endstop, accel=200, wait=False)
                #self._random_failure()
                #self._movequeues_wait_moves()

                self._set_selector_pos(0)
                self._trace_selector_move(None, extralength, 50, accel=accel)

                # Sync
                self.mmu_toolhead.sync_selector_to_extruder(self.extruder_name)
            else:
                if (length > 0):
                    self._set_filament_position(0)
                    self._trace_filament_move(None, length, speed, accel=accel, wait=False)

                self._set_filament_position(0)
                self._trace_filament_move(None, hominglength, homingspeed, homing_move=1, endstop_name=self.extruder_homing_endstop, accel=200, wait=False)

                self._set_filament_position(0)
                self._trace_filament_move(None, extralength, 50, accel=accel)
            ...
            raise gcmd.error("MMU_LOAD_TOOL: %s" % str(ee))

    cmd_MMU_UNLOAD_TOOL_help = "UNLoad tool"
    def cmd_MMU_UNLOAD_TOOL(self, gcmd):
        length = gcmd.get_float('LENGTH', self.calibrated_bowden_length)
        tool = gcmd.get_int('TOOL', 100)
        speed = gcmd.get_float('SPEED', 200)
        accel = gcmd.get_float('ACCEL', 100)
        try:
            self._movequeues_wait_moves()
            ...
            if tool == 1:
                ...
                self._set_selector_pos(0)
                self._trace_selector_move(None, -200, 50, homing_move=-1, endstop_name=self.extruder_homing_endstop, accel=500)
                ...
                self._set_filament_position(0)
                self._trace_filament_move(None, -200, 50, homing_move=-1, endstop_name=self.extruder_homing_endstop, accel=500)

                self._set_filament_position(0)
                self._trace_filament_move(None, -length, speed, accel=accel)

                # Sync
                self._sync_gear_to_extruder(1, servo=0, current=self._is_in_print(0))
        except MmuError as ee:
            raise gcmd.error("MMU_UNLOAD_TOOL: %s" % str(ee))
...
#################################
    ...
    def _trace_selector_move(self, trace_str, new_pos, speed=None, homing_move=0, endstop_name=None, wait=True, accel=None):
        if trace_str:
...
#################################
    ...
    def _adjust_selector_current(self, percent=100, reason=""):
         if self.selector_tmc and percent != self.selector_percentage_run_current and percent > 0 and percent < 200:
             self._log_info("Modifying MMU selector stepper run current to %d%% %s" % (percent, reason))
             self.gcode.run_script_from_command("SET_TMC_CURRENT STEPPER=stepper_mmu_selector CURRENT=%.2f" % ((self.selector_default_run_current * percent) / 100.))
             self.selector_percentage_run_current = percent
    ...
    def _restore_selector_current(self):
        if self.selector_tmc and self.selector_percentage_run_current != self.selector_restore_percent_run_current:
            self._log_info("Restoring MMU selector stepper run current to %d%% configured" % self.selector_restore_percent_run_current)
            self.gcode.run_script_from_command("SET_TMC_CURRENT STEPPER=stepper_mmu_selector CURRENT=%.2f" % self.selector_default_run_current)
            self.selector_percentage_run_current = self.selector_restore_percent_run_current
    ...
    def _wrap_gear_current(self, percent=100, reason=""):
        ...
        self.selector_restore_percent_run_current = percent # This will force restoration to this current not original (collision detection case)
        try:
            yield self
        finally:
            self.gear_restore_percent_run_current = 100
            self.selector_restore_percent_run_current = 100
            self._restore_gear_current()
