...
class MmuToolHead(toolhead.ToolHead, object):
    def __init__(self, config, homing_extruder):
        ...
        # by me
        self.selector_motion_queue = self.extruder_synced_to_selector = None
        ...
        self.last_flush_time = self.last_sg_flush_time = self.min_restart_time = 0.
    ...
    # by me Is selector rail synced to extruder (for in print syncing)
    def is_selector_synced_to_extruder(self):
        return self.selector_motion_queue is not None
    ...
    # by me
    def sync_selector_to_extruder(self, extruder_name):
        if self.extruder_synced_to_selector:
            self.sync_extruder_to_selector(None) # Mutually exclusive so unsync first

        printer_toolhead = self.printer.lookup_object('toolhead')
        printer_toolhead.flush_step_generation()
        self.flush_step_generation()
        gear_rail = self.get_kinematics().rails[0]
        ...
        if extruder_name:
            ...
            self.selector_motion_queue = extruder_name # We are synced!
        else:
            # Unsyncing
            if not self.selector_motion_queue: return
            for s, sk in zip(gear_rail.get_steppers(), self.prev_g_sk):
                s.set_stepper_kinematics(sk)
            gear_rail.set_trapq(self.get_trapq())
            g_pos = self.get_position()[1]
            ...
            self.selector_motion_queue = None
...
class MmuKinematics:
    def __init__(self, toolhead, config):
        ...
        # Setup "axis" rails
        self.axes = [('x', 'selector', False), ('y', 'gear', False)]
